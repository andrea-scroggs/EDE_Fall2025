...
##boxplots
O3.boxplot <- ggplot(EPAair, aes(x = as.factor(Year),
                                 y = Ozone)) +
  geom_boxplot() +
...
```
r squared is 0.1316 or 13% - only 13% of the variablility of PM2.5 is being explained by the Ozone data -- we need to figure out where else the variability is coming from because for an x-value the y-values can range from 0-75.
p-value is 2.2e-16
...
PM2.5_O3_scatter <- ggplot(EPAair, aes(x = Ozone,
                                       y = PM2.5)) +
  geom_point() +
  ...
  geom_text(x = 100,
            y = 75,
            label = expression('PM2.5=0*Ozone+15.64'))
print(PM2.5_O3_scatter)
...
EPAair_plots <- EPAair %>%
  ...
  na.omit()
EPAair_corrplots <- cor(EPAair_plots)
corrplot(EPAair_corrplots, method = 'ellipse')
corrplot.mixed(EPAair_corrplots, upper = 'ellipse')
...
ozonebyALL.regression <- lm(data = EPAair_corrplots,
...
ozone.BEST.regression <- lm(data = EPAair_corrplots,
                            PM2.5 ~ Ozone + Month + mean_Lng + mean_Lat)
summary(ozone.BEST.regression)
```
Exercise 5:
mean_Lng, Month, mean_Lat, and Ozone are the variables that make explain the relationship the best. 

Exercise 6:
Balance between adding more variables and the complexity of the model. 
Look at the adjusted r squared to understand your model and what would be the best to explain it. 
