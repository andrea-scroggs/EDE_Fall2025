...
#Update the column headers
colnames(gage_data_e1) = c(
  "agency_cd", "site_no", "datetime", "tz_cd",
  "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

#Tidy the data
gage_data_e1 <- gage_data_e1 %>% 
  select(datetime, discharge_cfs, gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data_e1,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
the_price <- the_website %>% html_nodes('')
the_capacity <- the_website %>% html_nodes('')
net_generation <- the_website %>% html_nodes('')
total_retail <- the_website %>% html_nodes('')
