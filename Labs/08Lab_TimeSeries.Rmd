...
Rainfall_BR_processed <-
  ...
  #my() changes the month name to a number month
...
#upwards trend line meaning increasing temperatures; up and down of the line means seasonality  

#plot for rainfall vs. time
ggplot(BR_complete,
       aes(x = Date,
           y = Rainfall)) +
  geom_line() +
...
#up and down movements of the line on the graph shows seasonality; pretty steady trend line
...
f_month <- month(BR_complete$Date)
f_year <- year(first(BR_complete$Date))

BR_rainfall_ts <- ts(BR_complete$Rainfall, start = c(1901, 1), frequency = 12)

BR_temperature_ts <- ts(BR_complete$Temperature_C, start = c(1901, 1), frequency = 12)
...
BR_rainfall_decomp <- stl(BR_rainfall_ts, s.window = 'periodic')
plot(BR_rainfall_decomp)

BR_temperature_decomp <- stl(BR_temperature_ts, s.window = 'periodic')
plot(BR_temperature_decomp)
...
BR_Rain_Components <- as.data.frame(BR_rainfall_decomp$time.series)

BR_Temp_Components <- as.data.frame(BR_temperature_decomp$time.series) 
...
BR_Temp_Components <- mutate(BR_Temp_Components,
        Observed = BR_complete$Temperatue_C,     
        Date = BR_complete$Date)
