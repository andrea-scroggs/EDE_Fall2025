...
knitr::kable(summary_PM25, caption = "Mean Particulates (2.5mm)", 
             col.names = c("County", "$\\mu g/m^3$"), digits = 2)
```
\newpage
```{r kable function, echo=FALSE, message=FALSE, warning=FALSE}

knitr::kable(summary_Ozone, caption = "Mean Ozone", 
             col.names = c("County", "µg/m3"), digits = 2)
```
...
```{r monthly ozone 2018, echo=FALSE, fig.cap='Monthly Ozone Concentration in 2018', message=FALSE, warning=FALSE, fig.height=4}
#boxplot for ozone levels by month in 2018
ozone2018 <- epa_data %>%
  filter(Year == "2018") %>%
  ggplot() +
  geom_boxplot(aes(x = factor(x = Month,
                              levels = 1:12,
                              labels = month.abb),
                   y = Ozone)) +
  labs(x = "Month", 
       y = "Concentration (µg/m3)",
       title = "Monthly Ozone Concentration in 2018") +
  theme(plot.margin = margin(2, 2, 2, 0))

print(ozone2018)
```

```{r monthly ozone 2019, echo=FALSE, fig.cap='Monthly Ozone Concentration in 2019', message=FALSE, warning=FALSE, fig.height=4}
#boxplot for ozone levels by month in 2019
ozone2019 <- epa_data %>%
  filter(Year == "2019") %>%
  ggplot() +
  geom_boxplot(aes(x = factor(x = Month,
  ...
  labs(x = "Month", 
       y = "Concentration (µg/m3)",
       title = "Monthly Ozone Concentration in 2019") +
  theme(plot.margin = margin(2, 2, 2, 0))
