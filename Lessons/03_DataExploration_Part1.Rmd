...
> Answer: When loading a package, it states in the console which objects are masked from the package you are trying to load. It also can show errors/conflicts of certain packages and the objects that are conflicting-R just chooses one for you of the conflicting objects/commands.
...
#read.csv(
#  file="/home/guest/ENVIRON 872/EDE_Fall2025/Data/Raw/USGS_Site02085000_Flow_Raw.csv",
...
USGS.flow.data <- read.csv(file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"), 
                           stringsAsFactors = TRUE)
```
...
> ANSWER: The README file is under the Metadata folder. The information in the document tells the reader the details on the methods used to collect the data, where it was collected, and when. It gives the necessary information needed if the study was to be reproduced and new data was collected.
...
#want to see rows 30000:30005 and columns 3, 8, and 14
USGS.flow.data[30000:30005, c(3, 8, 14)]

#class of specific columns
...
> Answer: The value of NA was inserted into the column
...
The code chunk below shows how we coerce R to set `site_no` to be a *factor*, not an integer. --> as.factor()
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #put in date in the format of the original data, including how it is separated
```
