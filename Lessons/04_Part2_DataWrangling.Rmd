...
NTL.phys.data.PeterPaul$sampledate <- as.Date(NTL.phys.data.PeterPaul$sampledate, 
                                              format = "%Y-%m-%d")
...
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, 
                                                  format = "%m/%d/%y")
...
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, 
                                             lakename, year4, daynum, month, 
                                             sampledate, depth:po4)
...
# Gather nutrient data into one column using pivot_longer -- more rows to the datatable
...
# `subset' function - going to replace the original data with the new data - `is.na' for a column --> will take out NA
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
# for each nutrient how many observations are there?
...
# Spread nutrient data into separate columns using pivot_wider -- more columns to a data table
