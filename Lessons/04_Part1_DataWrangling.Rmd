...
NTL.phys.data.TL1990.1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake", 
                                    year4 >= "1990", year4 <= "1999")

```
Question: Why don't we filter using row numbers?

> Answer: Because the data may not be in order and we would then have to pick out and look at every row to see which ones match the conditions we want. It would not be efficient. 
...
NTL.phys.data.temp.descending <- arrange(NTL.phys.data,
                                         desc(temperature_C))
...
```{r selecting columns}
NTL.phys.data.temps <- select(NTL.phys.data, lakename,
                              sampledate:temperature_C)
...
```{r mutating data}
NTL.phys.data.temps <- mutate(NTL.phys.data.temps, 
